...
| Short-put alone | 113% / **28.6** / 0.66 | 62% / **22.6** / **1.21** | **0.773** |
| **Range switch** | **266% / 44.3 / 0.66** | **100% / 31.4 / 1.03** | **0.552** *(4/4 samples)* |

**It beats the cash engine out of sample on all three measures at once — return 13 of 19 names, drawdown 11 of 19, Sharpe 13 of 19** — and it is the only structure here that does. Notably the per-name evidence gets *stronger* out of sample (13/11/13) than on full history (10/7/10), the opposite of the usual pattern. IREN **+2026%** full history and **+501%** OOS, SMR +565% / +143%.
...
- **The wide band degenerates on trend-persistent names.** OPEN enters at `tgt > 0.80` and, because exit needs `tgt < 0.20`, essentially never leaves: **96% of bars in stock, 958 rolls, zero exits**, returning −21%. That is a covered-stock position with heavy call churn, not a range switch. FIG is the mirror image at 57 rolls and 1 exit.
