...
**Slow/fast EMA ratio (`BiasEmaRatio`, default on).** After the ceiling, the bias is scaled by `clamp(slowBiasEMA / fastBiasEMA, 0.25, 2.0)` — a *mean-reverting tilt on the bias's own level*: when the fast EMA runs **above** the slow (the bias just spiked) the ratio is `< 1` and **damps** it; when the fast dips **below** the slow (a recent pullback in the bias) the ratio is `> 1` and **lifts** it. Unlike double-weighting the current window (which makes the level *more* reactive and hurt), this leans *against* recent bias moves and helps. Validated on a **broad random 500-name US-common-stock sample** (not just the basket): it raises Sharpe on **~60% of names** — strongest in the 50–100% HV band (67–71%) — and lifts return, at the cost of **~1–2 pp deeper drawdown on high-vol names**. So it's a *mild favorable trade*, not a free lunch (it gives back a sliver of the drawdown edge where the strategy earns it); on by default per the risk-appetite call. Set `BiasEmaRatio = false` for the plain ceiling'd bias.
