...
- **Smoothed:** the raw per-candle bias is jumpy (the persistence ratio moves fast on state transitions and the exponential is convex), so it's EMA-smoothed over `DynSmoothPeriod` to avoid whipsaw. Optionally a second, slower EMA (`DynSmoothSlow`, default **10** = off) can be added — `LongBias = min(fast, slow)` — so the lagging slow EMA caps *transient* spikes (the bias climbs only if the raw value is sustained). It's a **defensive dial**: `min(fast, slow) ≤ fast`, so it can only *trim* the bias, giving lower drawdown at the cost of Sharpe/return (HV 0.46→0.39 at slow=100). Left off by default so it doesn't undercut the timing modulation; raise it (e.g. 100) for a steadier, more conservative bias. `DynSlowMult` (default **1.0**) multiplies the slow EMA before the `min`, so `< 1` lowers that ceiling *proportionally* — a cleaner, scale-aware cap than a fixed `DynMax` (most meaningful with `DynSmoothSlow` raised). It's the same risk dial: `×0.75` buys a touch more Sharpe at higher drawdown, `×0.5` a touch more defense. No `DynMax`/slow/mult combination beats the neutral default on *both* the bull-only OOS and the 2022-inclusive full window — it's a Pareto frontier (aggression ⇄ drawdown), not a free lunch, so these stay levers, not defaults.

**Knobs** (all on `BankrollSimulator`, hand-set — *not* fitted to returns): `DynScale` (default **Exponential**), `DynBase` (bias at `z = 0`, default **1**), `DynDecay` (default **0.6**) / `DynSlope`, `DynSmoothPeriod` (default **10**), `DynSmoothSlow` (default **10** = off) / `DynSlowMult` (default **1.0** = no effect), `DynMin`/`DynMax` (default `[0, 15]`), `HvWindow`/`PersistWindow` (**60 / 63**), refs `HvRefMean`/`HvRefStd` (**57 / 34.6**), `PersRefMean`/`PersRefStd` (**0.072 / 0.010**), **`BiasBlend`** (default **1.0** = pure dynamic), **`DefensiveBias`** (default **0.5**), **`BiasTiming`** (default **on**, band `0.75–1.25`) and **`BiasNoInvert`** (default **off**) — see below. Set `DynamicLongBias = false` to fall back to a single fixed `BankrollSimulator.LongBias`.
...
**Timing modulation (`BiasTiming`, default on).** *Level for direction, window for timing.* The level-amplified skew above decides **how bullish** (a high `biasEma` rides the runs); `BiasTiming` then nudges it by the **latest 15-bar window vs its own long-run norm**: `adjEma ×= clamp(dynBias / biasEma, 0.75, 1.25)`. When the recent window runs hot vs its norm (accelerating) exposure ticks up; when it cools (decelerating) it ticks down. The band is deliberately **tight** — on a real trender the skew already clamps to max, so the modulation is moot there and can't de-lever the run; it only trims/adds in the *marginal* cases. That distinction matters: widen the band and it slides into the failure mode of a pure ratio model (which puts `biasEma` in the denominator and de-levers exactly the persistent winners). Kept tight, it's the one reformulation that **lifts OOS Sharpe at unchanged drawdown** (all-110 0.41→0.43 / HV 0.43→0.46, drawdown flat) *and* improves the big-winner compounds rather than cratering them (NVDA 928%→981%, SMCI 526%→733%, AVGO 473%→527%). Guarded off when `biasEma ≤ 0` (bearish regime — the level de-risks there already). Set `BiasTiming = false` for the plain level skew.
