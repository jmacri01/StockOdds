...
**LT-Bear penalty (`BearPenalty` ∈ [0,1], default 0).** The rolling skew builds a long lean fast in a bull regime and unwinds it slowly, so it can leave exposure skewed long *deep into a confirmed LT-Bear downtrend*. `BearPenalty` fixes that directly: **while LT is Bear**, it scales the long lean by `(1 − BearPenalty)` — **1** = no long bias in Bear (exposure follows the raw target), **0.75** = keep 25%, **0** = unchanged. It only shrinks a *positive* lean, never the protective negative skew. Unlike `BiasBlend`, it does nothing in a bull run — it bites *only* while LT is Bear, so it's a more *targeted* de-risk. Still a risk dial, not a free lunch: turn it up for sharper drawdown control at some Sharpe cost. Left at **0** by default.
