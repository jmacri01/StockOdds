...
- **Broad:** the **short-put leads** as the lowest-drawdown seller (+24%/15.0, ratio **1.59**), then PMCC + short puts (+22%/17.0, 1.29) and plain PMCC (+19%/17.0, 1.09, ≈ B&H). B&H is +37%/34.8 (1.06) and Cash +16%/16.7 (0.97); covered stock (+20%/19.6, 1.02) slips to B&H's level once its naked-call theta is gone.
...
- **Violent:** **B&H reclaims the ratio here** (+146%/55.8, **2.61**) — no overlay beats it once naked calls are gone. Covered stock (+103%/43.7, 2.36), the short-put (+85%/36.4, 2.33) and PMCC + short puts (+95%/40.7, 2.33) cluster just below, at ~⅔–¾ the drawdown. On this cohort the overlays' value is drawdown reduction, not return.
- **Basket (18, incl IREN):** the **short-put posts the top ratio** (+108%/24.3, **4.42**) at the shallowest drawdown; plain PMCC follows (+112%/27.7, 4.04), then PMCC + short puts (+114%/31.9, 3.57) and covered stock (+109%/30.6, 3.55). All still crush B&H (+140%/50.4, 2.78) and Cash (+85%/37.7, 2.25).
...
- **Short-put roll trigger — roll on *time* (hold to expiry), not on 50% profit.** The short-put caps net delta at `ShortPutCap = 0.50`, so above a 0.50 target it never rebalances on the drift band — the only roll triggers are time (`ShortRollDte`) and an optional profit target (`ShortProfitTarget`). Swept both: at the 14-DTE harvest, **rolling at expiry wins** (broad ratio 1.59, basket 4.42) and a 50%-profit rule is *counterproductive* — it lowers all three cohorts (broad 1.53, basket 3.57, decliners turn negative) and adds ~50% more rolls. At 14 DTE the put is already near expiry so holding captures the full theta ramp; a profit exit only fires on **rallies** (a losing put never reaches the target), where it pays a round-trip to re-sell a slower-decaying ATM put — pure added cost and re-exposure on the up-moves. (50%-profit management is a *long-DTE ≈ 45* discipline; it claws back a little basket ratio at 30–45 DTE, but that whole regime underperforms 14-DTE anyway.) **`ShortRollDte = 1`, `ShortProfitTarget = 0` (off) are the defaults.**
