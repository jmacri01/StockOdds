...
| **Short-put** *(fully clean — no naked legs; cash-securable)* | *(none)* | one short put at delta = min(target, **0.50**) — ATM, peak theta | 0 → 0.50 |
...
| *Buy & hold* | *+37% / 34.8* | — |
| *Cash (engine)* | *+16% / 16.7* | — |
| PMCC + short puts | +29% / 15.7 | +22% / 17.0 |
| PMCC | +25% / 15.6 | +19% / 17.0 |
| **Short-put** | +28% / 14.5 | **+25% / 15.1** |
| Covered stock | +28% / 18.0 | +20% / 19.6 |
...
| *Buy & hold* | *−23% / 46.9* | — |
| *Cash (engine)* | *−8% / 21.2* | — |
| PMCC + short puts | −1% / 17.0 | −6% / 18.3 |
| PMCC | −4% / 16.9 | −7% / 18.0 |
| **Short-put** | +3% / 16.0 | **+1% / 16.9** |
| Covered stock | −3% / 20.3 | −9% / 22.4 |
...
| *Buy & hold* | *+146% / 55.8* | — |
| *Cash (engine)* | *+87% / 36.9* | — |
| PMCC + short puts | +110% / 38.9 | +95% / 40.7 |
| PMCC | +92% / 41.2 | +79% / 42.3 |
| **Short-put** | +100% / 34.0 | **+88% / 35.8** |
| Covered stock | +120% / 41.1 | +103% / 43.7 |
...
| *Buy & hold* | *+140% / 50.4* | — |
| *Cash (engine)* | *+85% / 37.7* | — |
| PMCC + short puts | +131% / 29.8 | +114% / 31.9 |
| PMCC | +128% / 25.7 | +112% / 27.7 |
| **Short-put** | +123% / 23.4 | **+112% / 24.8** |
| Covered stock | +127% / 29.2 | +109% / 30.6 |


**Reading it (return ÷ max-DD).** With short calls now covered 1:1 (no naked calls — see below), the **fully-clean single short-put is the standout across the board**, and the call-covered structures come down to earth:
- **Broad:** the **short-put leads** as the lowest-drawdown seller (+25%/15.1, ratio **1.64**), then PMCC + short puts (+22%/17.0, 1.29) and plain PMCC (+19%/17.0, 1.09, ≈ B&H). B&H is +37%/34.8 (1.06) and Cash +16%/16.7 (0.97); covered stock (+20%/19.6, 1.02) slips to B&H's level once its naked-call theta is gone.
- **Decliners:** only the **short-put stays positive at mid** (+1%/16.8) where buy-&-hold loses −23% — the call-covered structures (PMCC −7%, covered stock −9%) turn mildly negative once the naked-call premium is removed. Short-put is the shallowest and the only one still clearly beating B&H.
- **Violent:** **B&H reclaims the ratio here** (+146%/55.8, **2.61**) — no overlay beats it once naked calls are gone. The short-put comes closest (+88%/35.8, **2.45**), then covered stock (+103%/43.7, 2.36) and PMCC + short puts (+95%/40.7, 2.33), all at ~⅔–¾ the drawdown. On this cohort the overlays' value is drawdown reduction, not return.
- **Basket (18, incl IREN):** the **short-put posts the top ratio** (+112%/24.8, **4.51**) at the shallowest drawdown; plain PMCC follows (+112%/27.7, 4.04), then PMCC + short puts (+114%/31.9, 3.57) and covered stock (+109%/30.6, 3.55). All still crush B&H (+140%/50.4, 2.78) and Cash (+85%/37.7, 2.25).
...
- **Short-put roll trigger — roll on *time* (hold to expiry) plus a 0.10 *delta-floor*; not on 50% profit.** The short-put caps net delta at `ShortPutCap = 0.50`, so above a 0.50 target it never rebalances on the drift band — the roll triggers are time (`ShortRollDte`), a delta floor (`ShortDeltaFloor`), and an optional profit target (`ShortProfitTarget`). The drift band already re-sells a rallied put once its delta decays ~0.30 below target (so at a 0.50 target, at ~0.20 delta), **but for mid-range targets (~0.15–0.40) `target − 0.30` is ≤ 0, so the band never fires on decay** — a put that rallies down to ~0.10 delta then sits nearly worthless until expiry. Adding a **0.10 delta-floor** (re-arm any short put whose delta decays below 0.10 while the target still wants more) closes that gap and is a clean win: broad ratio 1.59→**1.64**, basket 4.42→**4.51**, violent 2.33→**2.45**, decliners flat, drawdowns unchanged, for ~13 extra rolls/name. It beats a 50%-profit rule, which is *counterproductive* here (broad 1.53, basket 3.57, decliners negative, ~50% more rolls): a profit exit fires at ~0.30 delta with plenty of theta left and just churns, whereas the delta-floor is surgical — it only re-arms a put that's genuinely spent. (The floor is gated to the `ShortPut` structure; for PMCC/covered-stock the short put is only a >1.0 add-leg on top of a core that already supplies delta, so re-arming a decayed add-put there just churns.) **Defaults: `ShortRollDte = 1`, `ShortDeltaFloor = 0.10`, `ShortProfitTarget = 0` (off).**
...
**Bottom line:** at the tuned defaults (365-DTE LEAP, 14-DTE short legs, hold-20), and with **all short calls now covered 1:1 (no naked legs)**, the overlays still beat buy-&-hold on **return ÷ max-drawdown in three of four universes** — most of the upside at roughly half the drawdown, breakeven on decliners where B&H bleeds −23%. The clear standout is now the **single short-put** (one cash-securable put at delta ≤ 0.50, no naked legs of any kind): it leads on the broad set (1.64) and the flyer basket (4.51), stays positive on decliners, carries the shallowest drawdowns, and is the most cost-stable. **PMCC (0.80Δ)** remains the cleanest way to get stock-like delta from a defined-risk LEAP (basket 4.04); **PMCC + short puts** adds the >1.0 lean at the cost of naked-short-*put* margin (a delta-only picture — see the capital caveat above). Once the naked-call theta is removed, **buy-&-hold reclaims the ratio on the violent cohort** — there the overlays are a drawdown tool, not an outperformer. Two things to keep honest: this rests on **near-mid execution** and, more importantly, on the **14-DTE theta harvest whose front-week gamma/gap/assignment risk the model can't see** — so read the edge as a model ceiling, strongest and most-trustworthy in the *drawdown-reduction* it shows (consistent across the whole study) rather than the short-DTE return spike. Reproduce with `OptionsOverlaySimulator` over `BankrollResult.Positions`.
