...
| **Cash** (default) | 0.18 | **22.3%** |
| **Hold** (mirror B&H out of region) | 0.50 | 37.1% |
| *Buy & hold (reference)* | *0.50* | *deepest (always full-long)* |

The two durable contributions, both confirmed out-of-sample: **(1) screening** — a cap floor lifts every HV bucket and flips HV > 100 from −0.07 to +0.43 Sharpe (see [What to screen for](#what-to-screen-for)); and **(2) drawdown reduction** — the default Cash policy cuts mean OOS drawdown sharply (to ~22%, roughly 14 pts under Deploy) by sitting out whenever a name isn't in its both-bull region. Sub-$100M microcaps lose under *every* mode *and* under buy-&-hold, so the floor is the single most important gate.
...
|---|---|---:|---:|---|
| **`1` Cash** *(default)* | flatten to 0; redeploy the capital elsewhere | 0.18 | **22.3%** | running a **rotation portfolio**, or capital preservation leads — "go to cash" means "go find another stock to trade" |
| **`2` Hold** | mirror buy-&-hold (force full long) | 0.50 | 37.1% | you have **high conviction in the specific name** and do not want the region rule to exit a position you mean to ride through the dip |
| **`0` Deploy** | keep running the strategy | 0.50 | 35.9% | you want the raw signal everywhere; behaves ≈ Hold on Sharpe |

**Cash** gives by far the lowest drawdown (~−14 pts, to ~22%) but sacrifices ~0.32 Sharpe and ~19 pts of return by sitting in cash whenever either ratio is bearish (~70% of the time). **Hold** matches B&H (and Deploy) on Sharpe while keeping you invested — the right pick when exiting is not an option. Single-name-backtest caveat: the cash default **understates** any single name (it sits out instead of redeploying), so score a name's continuous full-history behaviour with `BearRegimeMode = 0`.
