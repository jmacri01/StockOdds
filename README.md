...
Because the engine now clamps to **150%** exposure (see [defaults](#5-from-target-to-position-the-overlay)), the strong-signal candles ask for a target above 1.0. Only the structures that **add** delta with short puts (straddle, covered stock) can express that; the **PMCC self-caps at its LEAP delta (~0.80)** and the short-put at 0.50. This is the whole reason the straddle now leads on the momentum names — it can run the leverage the engine wants, and the PMCC structurally cannot.
...
- **Broad:** the **straddle** leads (+31%/17.6, ratio **1.76**), then PMCC +28%/17.6 (1.58); the **short-put** is the lowest-drawdown seller (+22%/14.4, 1.51). All clear B&H (+37%/34.8, 1.06) and Cash (+16%/17.6, 0.91).
- **Decliners:** the standout is **covered stock (+3%)** — *positive at mid* where buy-&-hold loses −23% — plus the short-put holding ~flat (+0%/16.1) at the shallowest drawdown. Every structure beats B&H and Cash.
...
- **Basket:** the **straddle leads** — +162%/35.8 (**4.53**, the highest return, running the >1 delta the LEAP-capped PMCC can't), then PMCC +144%/32.8 (4.38) and covered stock +134%/38.5 (3.48). All crush B&H (+128%/48.2, 2.65) and Cash (1.64).
- **Cost sensitivity:** covered stock rolls the most contracts, so it loses the most from frictionless→mid; PMCC and the straddle are the most cost-stable.
